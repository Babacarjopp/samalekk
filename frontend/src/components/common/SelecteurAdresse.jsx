...
const SelecteurAdresse = ({ onAdresseChange }) => {
  ...
  const obtenirPositionGPS = async () => {
    ...
    try {
      console.log('🔍 Début de la demande de position GPS...');
      
      // Utiliser watchPosition pour forcer une nouvelle demande de permission
      const pos = await new Promise((resolve, reject) => {
        watchIdRef.current = navigator.geolocation.watchPosition(
          (position) => {
            console.log('✅ Position obtenue:', position.coords);
            // Arrêter le watch après la première position
            if (watchIdRef.current) {
              navigator.geolocation.clearWatch(watchIdRef.current);
              watchIdRef.current = null;
            }
          ...
          (error) => {
            ...
            // Arrêter le watch en cas d'erreur
            if (watchIdRef.current) {
              navigator.geolocation.clearWatch(watchIdRef.current);
              watchIdRef.current = null;
            }
      ...
      console.log('📍 Position capturée:', newPosition);
      ...
      try {
        ...
        console.warn('⚠️ Géocodage inverse échoué, utilisation des coordonnées brutes');
        if (onAdresseChange) {
          onAdresseChange({
      ...
      if (err.code === 1) {
        ...
        setErreur('Permission refusée. Pour réactiver : Allez dans les paramètres de votre navigateur > Confidentialité > Localisation > Autoriser ce site.');
      } else {
        const messages = {
          2: 'GPS non disponible. Vérifiez que votre GPS est activé dans les paramètres.',
          3: 'Délai dépassé. Le GPS prend trop de temps à répondre.',
        };
        setErreur(messages[err.code] || `Erreur GPS: ${err.message}`);
      }
    } finally {
      setChargement(false);
    }
          ...
          >
            {chargement ? (
              <>
                ...
                <span className="text-xs opacity-70">Veuillez autoriser la demande de position</span>
              </>
            ) : succes ? (
              <>
                ...
                <div className="flex-1">
                  ...
                  {permissionBloquee && (
                    <p className="text-sm mt-2">
                      <strong>Solution :</strong> Allez dans les paramètres de votre navigateur, 
                      trouvez ce site, et changez "Bloquer" en "Autoriser" pour la localisation.
                    </p>
                  )}
                </div>
              </div>
          ...
          {!succes && !erreur && (
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-center">
              <p className="text-blue-800 text-sm">
                ...
                Important : Quand vous cliquez, votre téléphone vous demandera l'autorisation. 
                Cliquez sur "Autoriser" ou "Permettre" pour que la géolocalisation fonctionne.
              </p>
            </div>
          )}
        </>
